...
// --- User-friendly User Selector Component (Unchanged) ---
...
// --- Main View Component (Logic Unchanged) ---
export default function AdminPtoTypesView() {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                                    ...
                                    <TableBody>
                                        {ptoTypes.map((ptoType) => (
                                            <TableRow key={ptoType.id}>
                                                ...
                                                <TableCell>
                                                    <div className="flex flex-wrap gap-1">
                                                        ...
                                                        {(ptoType.approver_users?.length || 0) > 0 && (
                                                            <Badge variant="default" className="gap-1"><Users className="h-3 w-3" /> {ptoType.approver_users?.length}</Badge>
                                                        )}
                                                    </div>
                                                </TableCell>
                                                <TableCell>
                                                ...
                                                <TableCell>
                                                    <Badge variant={ptoType.is_active ? 'default' : 'secondary'} className={cn(ptoType.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800', 'hover:bg-current')}>
                                                        {ptoType.is_active ? 'Active' : 'Inactive'}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell>
                                                    {ptoType.usage_stats && <div className="text-sm text-muted-foreground">{ptoType.usage_stats.requests_count} reqs</div>}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    <div className="flex items-center justify-end gap-1">
                                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => toggleActive(ptoType)}>
                                                            {ptoType.is_active ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                                                        </Button>
                                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEdit(ptoType)}><Edit className="h-4 w-4" /></Button>
                                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-600" onClick={() => handleDelete(ptoType)} disabled={ptoType.usage_stats && ptoType.usage_stats.requests_count > 0}>
                        ...
                        <div className="grid grid-cols-1 gap-x-8 border-t px-6 py-8 lg:grid-cols-5">
                            ...
                            <div className="mt-8 space-y-8 lg:col-span-2 lg:mt-0">
                                ...
                                <div className="space-y-2 rounded-md border p-4  ">
                                    ...
                                    <Input id="modal-color" type="color" value={formData.color} onChange={(e) => handleChange('color', e.target.value)} className="h-10 max-w-1/12 w-sm p-1 " />
                                </div>
                            </div>
                        </div>
            ...
            <AlertDialog open={showDeleteAlert} onOpenChange={setShowDeleteAlert}>
                <AlertDialogContent>
                    ...
                    <AlertDialogDescription>This will permanently delete the PTO type "{ptoTypeToDelete?.name}". This action cannot be undone.</AlertDialogDescription>
                    <AlertDialogFooter>
...
// NEW Helper component for a consistent toggle style
