...
export default function EditLesson({ module, lesson }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Edit',
            href: `/admin/modules/${module.id}/lessons/${lesson.id}/edit`,
        },
    ];

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.modules.lessons.update', [module.id, lesson.id]));
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${lesson.title} - ${module.title}`} />

            <div className="flex h-full max-h-screen flex-1 flex-col gap-6 rounded-xl p-6">
                <div className="flex items-center">
                    <Button asChild variant="ghost" size="sm" className="mr-4">
                        <Link href={route('admin.modules.lessons.show', [module.id, lesson.id])}>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4 text-sm text-muted-foreground">
                                    ...
                                    <div>
                                        <h4 className="font-medium text-foreground mb-1">Order Changes</h4>
                                        <ul className="list-disc list-inside space-y-1">
                                            {module.sequential_lessons ? (
                                                <>
                                                    <li>Changing order affects student progression</li>
                                                    <li>Consider prerequisite knowledge</li>
                                                    <li>Students must complete lessons in sequence</li>
                                                </>
                                            ) : (
                                                <>
                                                    <li>Order changes visual organization only</li>
                                                    <li>Students can still access lessons freely</li>
