...
//            ->unique();
//
//        // Debug: Log PTO types and their calendar settings
//        $ptoTypes = \App\Models\PtoModels\PtoType::select('id', 'name', 'code', 'show_in_department_calendar', 'is_active')->get();
//        Log::info("PTO Types calendar settings:", $ptoTypes->toArray());
//
...
//                $query->where('show_in_department_calendar', 1)
//                    ->where('is_active', 1);
//            })
...
//            ->get();
//
//        Log::info("Colleague requests found (after filtering): {$colleagueRequests->count()}");
//        Log::info("Colleague requests:", $colleagueRequests->pluck('id', 'pto_type_id')->toArray());
//
...
//            ->whereIn('status', ['approved', 'pending'])
//            ->whereHas('ptoType', function ($query) {
//                $query->where('is_active', 1); // Only check if PTO type is active
//            })
//            ->where('start_date', '>=', now()->subMonths(2)) // Last 2 months
//            ->where('end_date', '<=', now()->addMonths(4))   // Next 4 months
//            ->get();
//
//        Log::info("User own requests found: {$userOwnRequests->count()}");
//
...
//            });
//
//        Log::info("Total requests returned: {$allRequests->count()}");
//
